...
const calculateMultiSampleRateFilter = (currentResidual, bandIdx, config) => {
    ...
    if (decFilterOriginalLen === 0) {
        throw new Error(`Decimation filter is empty (length 0) for non-zero length band ${bandIdx}. This should not happen.`);
    }
...
const calculateMultirate = (impulseResponse, config) => {
    ...
    for (let bandIdx = 0; bandIdx < bandsToProcess; bandIdx++) {
        try {
            ...
            throw new Error(`Failed during multi-sample rate processing for band ${bandIdx}: ${error.message}`);
        }
    }
    ...
    if (configToUse) {
        try {
            ...
            if (mangledFilter.length !== expectedOutputLength) {
                console.warn(`WARNING: XT32 (${type}) decimation output length (${mangledFilter.length}) does not match expected (${expectedOutputLength}). Input length was ${inputLength}. Padding or truncation might occur implicitly.`);
            }
...
const filterConfigs = {
    xt32Sub: {
        ...
        decFiltersInfo: [
            ...
            { phases: decomposeFilter(decFilterXT32Sub93_taps, DECIMATION_FACTOR), originalLength: decFilterXT32Sub93_taps.length }
        ],
...
class UPNPDiscovery {
    ...
    discover() {
        return new Promise((resolve, reject) => {
            ...
            this.socket.on('message', (msg, rinfo) => {
                ...
                if (response.toUpperCase().startsWith('HTTP/1.1 200 OK') && response.toUpperCase().includes('LOCATION:')) {
                ...
                try {
                    ...
                    this.SEARCH_TARGETS.forEach(target => {
                        ...
                        this.socket.send(searchRequest, 0, searchRequest.length, this.SSDP_PORT, this.SSDP_MULTICAST_ADDR, (err) => {
                            if (err) {
                                console.error(`Error sending M-SEARCH for target ${target} via ${localAddress}: ${err}`);
                            }
                        });
                    });
                } catch (bindErr) {
                    console.error(`Error setting up discovery socket options (bind/multicast) on ${localAddress}:`, bindErr);
                    if (this.active) {
                        this.active = false;
                        clearTimeout(discoveryTimer);
                        try { this.socket.close(); } catch (e) { }
                    ...
                    res.on('end', () => { 
                        parseStringPromise(data)
                            .then(parsed => {
                                ...
                                if (!device) {
                                    throw new Error('Invalid device description XML structure (root.device missing).');
                                }
                                resolve({
    ...
    static async interactiveDeviceSelection(devices) {
        ...
        const choices = devices.map((device, index) => ({
            name: `[${index + 1}] ${device.friendlyName && !/unknown/i.test(device.friendlyName) ? device.friendlyName : device.modelName || 'Unknown Model'} (${device.manufacturer || 'Unknown Manuf.'}) - ${device.address}`,
            value: index
        }));
...
async function _connectToAVR(ip, port, timeout, purpose) {
    return new Promise((resolve, reject) => {
        ...
        connectionTimeoutTimer = setTimeout(() => {
            console.error(`Connection to ${ip}:${port} for ${purpose} timed out after ${timeout}ms (manual timer).`);
            client.destroy();
        ...
        client.once('error', err => {
            cleanup();
            console.error(`${purpose.charAt(0).toUpperCase() + purpose.slice(1)} connection error to ${ip}:${port}: ${err.message} (Code: ${err.code})`);
            reject(new Error(`Connection error: ${err.message}`));
        });
        client.once('timeout', () => {
            cleanup();
            console.error(`${purpose.charAt(0).toUpperCase() + purpose.slice(1)} connection to ${ip}:${port} timed out (net.socket timeout event).`);
            client.destroy();
...
async function _sendRawAndParseJsonHelper(socket, hexWithChecksum, label, commandTimeout, bufferLimitBytes, errorContext = '') {
    return new Promise((resolve, reject) => {
            ...
            if (jsonStart !== -1 && jsonEnd !== -1 && jsonEnd > jsonStart) {
                ...
                try {
                    ...
                    if (buffer.length > bufferLimitBytes) {
                        cleanup(new Error(`[${label}] ${errorContext} Response buffer exceeded ${bufferLimitBytes / 1024}KB without valid JSON.`));
                    }
                }
            } else {
                if (buffer.length > bufferLimitBytes) {
                    cleanup(new Error(`[${label}] ${errorContext} Response buffer exceeded ${bufferLimitBytes / 1024}KB without JSON start/end markers.`));
                }
            }
        };
        const onError = (err) => {
        ...
        commandTimer = setTimeout(() => {
            console.error(`[${label}] ${errorContext} Command timed out after ${commandTimeout}ms waiting for JSON response.`);
...
async function fetchModelFromGoform(ipAddress) {
    ...
    return new Promise((resolve) => {
        ...
        const req = http.request(url, options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    const genericPatterns = /receiver|network (audio|av)|(av|media) (server|renderer|player)/i;
                    if (!modelName || genericPatterns.test(modelName)) {
                        if (friendlyName && friendlyName.length > 3 && !genericPatterns.test(friendlyName)) {
        ...
        req.on('error', (e) => {
            if (e.code === 'ECONNREFUSED' || e.code === 'EHOSTUNREACH' || e.code === 'ENETUNREACH') {
            } else {
...
async function runFullDiscoveryAndSave(interactive = true) {
    ...
    try {
        ...
        console.log(`Found ${uniqueIPs.length} unique IP address(es) associated with potential AVRs.`);
    ...
    if (interactive) {
        ...
        if (promptForModel) {
            const modelPrompt = await inquirer.prompt([{
                ...
                message: 'Please enter the correct AVR Model Name e.g., SR6011, X3800H (last 6 digits are evaluated):',
...
async function initializeApp() {
    console.log('--------------------------------');
    console.log('  A1 Evo Acoustica v6.3b by OCA');
    console.log('--------------------------------');
            ...
            if (method === 'POST' && pathname === '/api/save-oca') {
                ...
                req.on('end', async () => {
                    ...
                    try {
                        ...
                        if (!ocaData || typeof ocaData !== 'object' || !ocaData.channels ||
                            !receivedOcaFilename || typeof receivedOcaFilename !== 'string' ||
                            typeof logHtml !== 'string') {
                            throw new Error("Invalid payload received. Missing ocaData, filename, or logHtml.");
                        }
                        if (!receivedOcaFilename.toLowerCase().endsWith('.oca')) {
                            throw new Error("Invalid OCA filename received. Must end with .oca");
                        }
                        safeOcaFilename = path.basename(receivedOcaFilename.replace(/[\/\\]/g, '_'));
                        ...
                        console.error(`[Server] Error processing /api/save-oca request: ${error.message}`);
...
function findRewPath() {
    ...
    if (platform === 'win32') {
        ...
        console.warn("Automatic REW path detection on Linux/Unix is limited. Assuming 'roomeqwizard' is in PATH.");
        return 'roomeqwizard';
    }
...
async function getAvrInfoAndStatusForTransfer(socket) {
    ...
    try {
        const infoJson = await _sendRawAndParseJsonHelper(socket, '54001300004745545f415652494e460000006c', 'GET_AVRINF', commandTimeout, 8 * 1024, 'Transfer');
        const statusJson = await _sendRawAndParseJsonHelper(socket, '54001300004745545f41565253545300000089', 'GET_AVRSTS', commandTimeout, 8 * 1024, 'Transfer');
    ...
    return new Promise((resolve, reject) => {
        ...
        client.on('data', (data) => {
            ...
            if (isPowerConfirmed && !hasHandledPreset && presetMatch) {
                ...
                rl.question('Select preset to store calibration (1 or 2, Enter for current): ', (answer) => {
                    if (rl) {
                        rl.close();
                        rl = null;
                    } else { return; }
...
const addCheckSum = hex => {
    if (typeof hex !== 'string' || hex.length % 2 !== 0) {
        ...
        throw new Error(`Hex string for checksum must have even length. Received length ${hex?.length}. Start: ${preview}...`);
    }
    let checksum = 0;
...
function getChannelTypeByte(commandId, multEqType, isGriffin = false) {
    ...
    if (isGriffin && entry.griffin === null) {
        console.warn(`⚠ Griffin channel byte requested but not available for ${commandId}, falling back to standard mapping...`);
    }
    if (multEqType === 'XT32') {
    ...
    return async (payload, label, { 
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                if (addChecksum) {
                    ...
                    if (Buffer.isBuffer(payload)) {
                        ...
                        return reject(new Error(`[${label}] Payload must be a Buffer or hex string when addChecksum is false.`));
                    }
                }
            } catch (e) {
            ...
            const dataHandler = (data) => {
                ...
                if (textResponse.includes('NAK') || textResponse.includes('NACK') || textResponse.includes('ERROR')) {
            ...
            timer = setTimeout(() => {
                if (!cleanedUp) {
                    cleanup(!expectAck, expectAck ? 'No response/ACK received' : 'Command sent (no ACK expected)');
                }
            }, effectiveTimeout);
...
function mapChannelIdForSetDat(id) {
    switch (id) {
        ...
        case 'SWLFE2SP': return 'SW1';
        case 'SWLEFT2SP': return 'SW1';
        case 'SWRIGHT2SP': return 'SW2';
        case 'SWFRONT2SP': return 'SW1';
        case 'SWBACK2SP': return 'SW2';
        case 'SWLFE3SP': return 'SW1';
        case 'SWLEFT3SP': return 'SW1';
        case 'SWRIGHT3SP': return 'SW2';
        case 'SWFRONTLEFT3SP': return 'SW1';
        case 'SWFRONTRIGHT3SP': return 'SW2';
        case 'SWREAR3SP': return 'SW3';
        case 'SWLFE4SP': return 'SW1';
        case 'SWFRONTLEFT4SP': return 'SW1';
        case 'SWFRONTRIGHT4SP': return 'SW2';
        case 'SWBACKLEFT4SP': return 'SW3';
        case 'SWBACKRIGHT4SP': return 'SW4';
...
function processFilterDataForTransfer(channelFilterData, multEqType, lookupChannelId) {
    ...
    if (multEqType === "XT32") {
        ...
        if (expectedLength > 0) {
            if (processedFilter.length !== expectedLength) {
                console.warn(`      WARNING: Input filter length for ${multEqType} channel ${lookupChannelId} is ${processedFilter.length}, expected ${expectedLength}. AVR might reject or truncate.`);
            }
            if (processedFilterLV.length !== expectedLength) {
                console.warn(`      WARNING: Input filterLV length for ${multEqType} channel ${lookupChannelId} is ${processedFilterLV.length}, expected ${expectedLength}. AVR might reject or truncate.`);
            }
        } else {
            console.warn(`      Cannot determine expected length for non-XT32 type "${multEqType}". Sending filter as is.`);
        }
    }
    return {
...
async function runCalibrationTransfer(targetIp, basePathForOcaSearch) {
    ...
    try {
        ...
        if (!filterData || !filterData.channels || !Array.isArray(filterData.channels)) throw new Error(`Internal Error: Invalid filterData during comparison.`);
        if (!activeChannels || !Array.isArray(activeChannels)) throw new Error(`Could not retrieve valid active channel list from AVR for comparison.`);
        if (!avrStatus || !avrStatus.AssignBin || !avrStatus.AmpAssign) throw new Error(`Could not retrieve valid AssignBin/AmpAssign status from AVR for comparison.`);
        const ocaChannelNames = filterData.channels.map(ch => ch.commandId);
        const normalizedOcaChannelIds = new Set(ocaChannelNames.map(id => mapChannelIdForSetDat(id)));
        ...
        try {
            ...
            await send(enterAudyHex, 'ENTER_AUDY', { timeout: TRANSFER_CONFIG.timeouts.enterCalibration, expectAck: true, addChecksum: false });
        ...
        if (channelsToSendSorted.length === 0 && activeChannels.length > 0) { 
            ...
            for (const tc of TRANSFER_CONFIG.targetCurves) {
                ...
                for (const channelInfo of channelsToSendSorted) {
                    ...
                    const coeffsToSend = (tc === '01') ? processedDataForChannel.filter : processedDataForChannel.filterLV;
                    try {
        ...
        console.log(`Your receiver "${cachedAvrConfig?.targetModelName || targetIp}" is now updated.`);
        console.log(`\nReminder:`);
